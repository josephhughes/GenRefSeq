...
if available ,first publication year, number of unidentifed nucleotides(N) and write these to file
...
#From the command line the input (genbankfile with all sequences and associated data for a taxonomic id),
# output(tab delimited and fasta format file) and length of 'near' full genome
...
for gen_record in SeqIO.parse(open(args.input,"r"), "genbank"):
  ...
  #Want to filter out the records which have the terms within the description shown below
	...
	term =re.search (r'vector|Patent|clone|Chimeric|chimeric|VACCINE|Vaccines|vaccine|attenuated|Modified|adapted|fusion|Tr-subtype|Recombinant Hepatitis C virus|NPHV|UNVERIFIED|replicon|Synthetic|therapy|Mutant|SYNTHESIZED|construct|CONJUGATES', des_str)
...
#The fasta file contains accession number, collection date if available (or publication) and DNA sequence.
